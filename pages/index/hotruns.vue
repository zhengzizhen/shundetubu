<template>
	...
	<view class="dc_bg">
		...
		<view class="dc_header pd30">
			...
			<view v-if="loadings">
				<p style="margin-top: 180rpx;color: white;text-align: center;">暂无活动</p>
			</view>

			<!-- //小图 -->
			<view v-if="list.length!=0" @click="toDetails(item.id)" v-for="(item,index) in list" :key="index">
	...
	export default {
		...
		methods: {
			async getlists() {
				...
				this.list.length == 0 ? this.loadings = true : ''
			...
			async getSeach(url) {
				...
				this.addresslist = res.data.data.search_time
